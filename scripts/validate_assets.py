#!/usr/bin/env python3
"""Lightweight repository validation for visual assets."""
...
ASSET_ROOTS = [
    "brand", "banners", "illustrations", "animations", "icons", "badges",
    "backgrounds", "project-covers", "screenshots", "source", "exports", "templates",
]
ALLOWED_SUFFIXES = {
    ".svg", ".png", ".jpg", ".jpeg", ".webp", ".gif", ".json", ".md",
    ".yml", ".yaml", ".drawio", ".psd", ".ai", ".fig", ".sketch", ".aep",
    ".mp4", ".mov", ".webm", ".txt",
}
...
def main() -> int:
    ...
    required = [
        ...
        "manifest/assets.yml", "docs/STYLE_GUIDE.md", "docs/ASSET_SPEC.md",
    ]
    for rel in required:
    ...
    for root_name in ASSET_ROOTS:
        ...
        for path in root.rglob("*"):
            ...
            if path.name not in {"README.md", ".gitkeep"} and not NAME_RE.match(path.name):
                errors.append(f"name must be lowercase kebab/safe-case: {rel}")
            size = path.stat().st_size
            if size > MAX_HARD_BYTES:
                errors.append(f"file exceeds 50 MiB hard policy: {rel}")
            elif size > MAX_NORMAL_BYTES:
                warnings.append(f"large file; consider Git LFS/release assets: {rel}")
